...
if [[ BUILDS_FINISHED -eq 4 ]]; then
    ...
    if [ "$TREE_NAME" == "arm-soc" ] || [ "$TREE_NAME" == "mainline" ] || [ "$TREE_NAME" == "stable" ] || [ "$TREE_NAME" == "rmk" ] || [ "$TREE_NAME" == "tegra" ]; then
        ...
        curl -XPOST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "drm-tip", "kernel": "'$GIT_DESCRIBE'", "build_report": 1, "format": ["txt"], "send_to": ["daniel@ffwll.ch", "dri-devel@lists.freedesktop.org", "fellows@kernelci.org"], "delay": 60}'
        curl -XPOST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "drm-tip", "kernel": "'$GIT_DESCRIBE'", "boot_report": 1, "format": ["txt"], "send_to": ["daniel@ffwll.ch", "dri-devel@lists.freedesktop.org", "fellows@kernelci.org"], "delay": 12600}'
    else
